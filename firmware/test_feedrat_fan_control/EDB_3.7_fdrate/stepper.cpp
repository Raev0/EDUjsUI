...
void stepper_permit()
{
unsigned char step_s[13];

for(int i=0;i<=12;i++)
step_s[i]=EEPROM.read(100+i);
unsigned char permit;
unsigned char permit_w;
//TEST_SOUND();
asm volatile(
"step_s_s:"         "\n\t"
"mov r20,%A1 "      "\n\t"
...
if(step_s[12]==87)permit++;
EEPROM.write(111,permit);
//delay(300);
}
 ...
 if( endstop_x_hit || endstop_y_hit || endstop_z_hit) {
   ...
   if(endstop_x_hit) {
     SERIAL_ECHOPAIR(" X:",(float)endstops_trigsteps[X_AXIS]/axis_steps_per_unit[X_AXIS]);
       //zct:LCD_MESSAGEPGM(MSG_ENDSTOPS_HIT "X");
   }
   if(endstop_y_hit) {
     SERIAL_ECHOPAIR(" Y:",(float)endstops_trigsteps[Y_AXIS]/axis_steps_per_unit[Y_AXIS]);
       //zct:LCD_MESSAGEPGM(MSG_ENDSTOPS_HIT "Y");
   }
   if(endstop_z_hit) {
     SERIAL_ECHOPAIR(" Z:",(float)endstops_trigsteps[Z_AXIS]/axis_steps_per_unit[Z_AXIS]);
       //zct:LCD_MESSAGEPGM(MSG_ENDSTOPS_HIT "Z");
   }
   SERIAL_ECHOLN("");
   endstop_x_hit=false;
   endstop_y_hit=false;
...
//      #endif
  ...
  if (current_block != NULL) {
	 ...
	 * lag to allow it work with without needing NOPs */
      ...
      #ifndef ADVANCE
        counter_e += current_block->steps_e;
		if(w_h_text_pause==2)counter_e=0;
        if (counter_e > 0) {
